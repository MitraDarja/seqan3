 ...
 */
template <std::ranges::view urng_t, std::ranges::view urng_t2>
class minimiser_view : public std::ranges::view_interface<minimiser_view<urng_t, urng_t2>>
{
private:
    ...
    static_assert(std::totally_ordered<reference_t<urng_t>>, "The reference type of the underlying range must model "
                                                             "std::totally_ordered.");
    //!\brief True if two ranges are given.
    bool two{false};
    ...
    template <typename rng_t, typename rng_t2>
...
public:
    ...
    * \param[in] w_         The number of values in one window.
    */
    minimiser_view(urng_t urange_, urng_t2 urange2_, uint32_t const w_) : two{true}, urange{std::move(urange_)},
    ...
    */
    ...
    //!\cond
     requires std::ranges::viewable_range<rng_t> &&
              std::constructible_from<urng_t, ranges::ref_view<std::remove_reference_t<rng_t>>>
    //!\endcond
    minimiser_view(rng_t && urange_, rng_t2 && urange2_, uint32_t const w_) : two{true},
    ...
    {
        if (two)
            return window_iterator<urng_t, urng_t2>{std::ranges::begin(urange), std::ranges::end(urange),
    ...
    auto begin() const
    //!\cond
        requires seqan3::const_iterable_range<urng_t> && seqan3::const_iterable_range<urng_t2>
    //!\endcond
    {
        if (two)
            return window_iterator<urng_t const, urng_t2 const>{std::ranges::begin(urange), std::ranges::end(urange),
                                                                std::ranges::begin(urange2), window_values_size};
        return window_iterator<urng_t const, urng_t2 const>{std::ranges::begin(urange), std::ranges::end(urange),
                                                            window_values_size};
    }
...
//!\brief A deduction guide for the view class template.
template <std::ranges::viewable_range rng_t, std::ranges::viewable_range rng_t2>
minimiser_view(rng_t &&, rng_t2 &&, uint32_t const & window_values_size) -> minimiser_view<std::ranges::all_view<rng_t>, std::ranges::all_view<rng_t2>>;
...
struct minimiser_fn
{
     ...
     */
     ...
     template <std::ranges::range urng_t2>
     constexpr auto operator()(uint32_t const & window_values_size, urng_t2 && urange2) const
     {
     ...
     * \returns                         A range of converted values.
     */
    template <std::ranges::range urng_t, std::ranges::range urng_t2>
    constexpr auto operator()(urng_t && urange, uint32_t const & window_values_size, urng_t2 && urange2) const
    {
        static_assert(std::ranges::viewable_range<urng_t>,
            "The range parameter to views::minimiser cannot be a temporary of a non-view range.");
        static_assert(std::ranges::forward_range<urng_t>,
 ...
 * \tparam urng_t2                  The type of the range second being processed. See below for requirements. [template
 *                                  parameter is omitted in pipe notation]
