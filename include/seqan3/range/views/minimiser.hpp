 ...
 */
template <std::ranges::view urng_t, typename urng2_t = std::ranges::empty_view<int>>
...
private:
    ...
    static constexpr bool urange2_is_given = !std::same_as<urng2_t, std::ranges::empty_view<int>>;
...
public:
    ...
    */
    minimiser_view(urng_t urange_, uint32_t const w_) :
        urange{std::move(urange_)}, window_values_size{w_}
    {}
    ...
    * \param[in] w_         The number of values in one window.
    */
    minimiser_view(urng_t urange_, urng2_t urange2_, uint32_t const w_) :
    ...
    */
    ...
    minimiser_view(rng_t && urange_, rng2_t && urange2_, uint32_t const w_) :
        ...
        urange2{std::views::all(std::forward<rng_t>(urange2_))},
    ...
    auto begin()
    {
        return window_iterator<urng_t, urng2_t>{std::ranges::begin(urange),
    ...
    //!\endcond
    {
        return window_iterator<urng_t const, urng2_t const>{std::ranges::begin(urange),
                                                            std::ranges::end(urange),
                                                            std::ranges::begin(urange2),
                                                            window_values_size};
    }
